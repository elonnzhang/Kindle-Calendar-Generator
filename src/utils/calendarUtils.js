/**
 * Browser-compatible calendar generator
 */
 ...
 */
export function downloadSVG(year, month, width, height) {
  const svg = generateCalendarSVG(year, month, width, height);
  const blob = new Blob([svg], { type: 'image/svg+xml' });
  const url = URL.createObjectURL(blob);
  const a = document.createElement('a');
  a.href = url;
  a.download = `calendar_${year}_${month.toString().padStart(2, '0')}.svg`;
